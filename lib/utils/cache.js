...
const extCache = (SequelizeGuard) => {
  ...
  SequelizeGuard.prototype._bindCacheCallbacks = function() {
    let guard = this;

    guard.onRolesCreated(async function (roles) {
      ...
      let mappedRoles = mappedRolesToIds(roles);
      let cache = await guard.getCache();
    ...
    guard.onRolesDeleted(async function (roles) {
      let cache = await guard.getCache();
    ...
    guard.onPermsCreated(async function (perms) {
      let mappedPerms = mappedPermsToIds(perms);
      let cache = await guard.getCache();
      cache.setPerms({
        ...cache.getPerms(),
    ...
    guard.onPermsAddedToRole(async function (role) {
      let roles = [role];
      let mappedRoles = mappedRolesToIds(roles);
    ...
    guard.onPermsRemovedFromRole(async function (role) {
      let roles = [role];
      let mappedRoles = mappedRolesToIds(roles);
...
function guardMigrationErrShow() {
  console.log('====================================');
  console.log(
    '\tTables for Guard not created, make sure you have run migrations or called the sync method'
  );
  console.log('====================================');
}
