...
module.exports = (SequelizeGuard) => {
  ...
  * Assign role to user
  ...
  SequelizeGuard.prototype.assignRole = function (user, role) {
    return this.makeRole(role).then(({ role }) => {
  ...
  * Remove roles from association with user user
  ...
  SequelizeGuard.prototype.getUserRoles = async function (
    ...
    return _guard.getUserCache().then((cache) => {
      ...
      if (cacheRoles) {
        if (options.json) {
          return cacheRoles.map((perm) => perm.toJSON());
        }
        return cacheRoles;
      }
      ...
      return user.getRoles().then((fetchedRoles) => {
        if (_guard._options.userCache) {
          cache.set(cacheKey, fetchedRoles, _guard._options.userCacheTtl);
        }
        if (options.json) {
          return fetchedRoles.map((perm) => perm.toJSON());
        }
        return fetchedRoles;
      });
    });
